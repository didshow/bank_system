...
contract Universe {
    ...
    function rely(address usr) external auth { require(live == 1, "Universe/not-live"); wards[usr] = 1; }
    function deny(address usr) external auth { require(live == 1, "Universe/not-live"); wards[usr] = 0; }
    modifier auth {
    ...
    function withdraw(address src,address to, uint256 amount) external {
        require(live == 1, "Universe/not-live");
        require(totalStars >= amount || stars[msg.sender] >= amount, "Universe/not-enough-stars");
